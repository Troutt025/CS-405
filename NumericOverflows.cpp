...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
	...
	{
		//If statement to test if result is still greater than the max numeric limit of the variable - increment. If so, adding them together would cause overflow.
		if((increment > 0) && (result > std::numeric_limits<T>::max() - increment)) {
			//Adding variables together should not cause them to be the minimum numeric limit. This will be our return to tell the user an overflow happened.
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
	...
	for (unsigned long int i = 0; i < steps; ++i)
	{
		//If statement to test if result is still less than the numeric minimum + decrement. If so, subtracting them would cause underflow.
		if((decrement > 0) && (result < std::numeric_limits<T>::min() + decrement)) {
			//Subtracting variables should not cause the result to be the maximum numeric limit. This will be our return to tell the user an underflow happened.
...
void test_overflow()
{
	...
	//If statement to check for overflow variable and tell the user.
...
void test_underflow()
{
	...
	//If statement to check for underflow variable and tell the user.
